...
local Templates = {
    ...
    Window = {
        ...
        MobileLayout = "Auto",
        MobileSize = UDim2.fromOffset(520, 330),
        MobileWidth = 0.7,
        MobileHeight = 0.86,
        MobileMinScale = 0.6,
        MobileMaxScale = 1,
...
function Library:SetDPIScale(DPIScale: number, FromLayout: boolean?)
    if not FromLayout and Library.MobileLayout and Library.MobileLayout.Active then
        Library.MobileLayout.ManualScale = true
    end
...
do
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local SwitchCorner = New("UICorner", {
            CornerRadius = UDim.new(1, 0),
            Parent = Switch,
        })
        New("UIPadding", {
        ...
        local BallCorner = New("UICorner", {
            CornerRadius = UDim.new(1, 0),
            Parent = Ball,
        })

        Toggle.StyleExplicit = Info.Style ~= nil

        function Toggle:SetStyle(Style: string)
            Toggle.Style = Style == "Square" and "Square" or "Round"

            local Square = Toggle.Style == "Square"
            SwitchCorner.CornerRadius = Square and UDim.new(0, 0) or UDim.new(1, 0)
            BallCorner.CornerRadius = Square and UDim.new(0, 0) or UDim.new(1, 0)
            SwitchGradient.Enabled = Square
            BallShadow.Visible = Square
            Switch.Size = Square and UDim2.fromOffset(38, 20) or UDim2.fromOffset(32, 18)
            Button.Size = UDim2.new(1, 0, 0, Square and 20 or 18)
            Label.Size = UDim2.new(1, Square and -46 or -40, 1, 0)
        ...
        Toggle:SetStyle(Info.Style or Library.ToggleStyle)
        ...
        function Toggle:Display()
            ...
            if Square then
                ...
                SwitchStroke.Transparency = Toggle.Value and 1 or (Toggle.Disabled and 0.9 or 0.8)
                SwitchGradient.Color = Toggle.Value
                    and ColorSequence.new(Color3.fromRGB(205, 205, 205), Color3.fromRGB(255, 255, 255))
                    or ColorSequence.new(Color3.fromRGB(80, 80, 80), Color3.fromRGB(138, 138, 138))
            ...
            BallShadow.BackgroundTransparency = Toggle.Disabled and 0.85 or 0.6

            if Toggle.Disabled then
                Label.TextTransparency = 0.8
                Ball.AnchorPoint = Vector2.new(Offset, 0)
                Ball.Position = UDim2.fromScale(Offset, 0)
                BallShadow.AnchorPoint = Vector2.new(Offset, 0)
                BallShadow.Position = UDim2.new(Offset, 0, 0, 1)
            ...
            TweenService:Create(BallShadow, Library.TweenInfo, {
                AnchorPoint = Vector2.new(Offset, 0),
                Position = UDim2.new(Offset, 0, 0, 1),
            }):Play()
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        local FillGradient = New("UIGradient", {
            Color = ColorSequence.new(Color3.fromRGB(176, 176, 176), Color3.fromRGB(255, 255, 255)),
            Enabled = false,
            Parent = Fill,
        })
        ...
        table.insert(Library.Corners, BarCorner)

        local FillCorner = New("UICorner", {
            CornerRadius = UDim.new(0, Library.CornerRadius / 2),
            Parent = Fill,
        })
        table.insert(Library.Corners, FillCorner)

        Slider.StyleExplicit = Info.Style ~= nil

        function Slider:SetStyle(Style: string)
            Slider.Style = Style == "Square" and "Square" or "Default"

            local Square = Slider.Style == "Square"
            for _, Corner in { BarCorner, FillCorner } do
                local Index = table.find(Library.Corners, Corner)
                if Square and Index then
                    table.remove(Library.Corners, Index)
                elseif not Square and not Index then
                    table.insert(Library.Corners, Corner)
                end
                Corner.CornerRadius = Square and UDim.new(0, 0) or UDim.new(0, Library.CornerRadius / 2)
            end

            FillGradient.Enabled = Square
            Bar.Size = UDim2.new(1, 0, 0, Square and 18 or 15)
            Holder.Size = UDim2.new(1, 0, 0, Info.Compact and (Square and 18 or 15) or (Square and 35 or 33))
        ...
        Slider:SetStyle(Info.Style or Library.SliderStyle)
            ...
            if CustomDisplayText then
                ...
                elseif Info.HideMax then
                    ...
                    DisplayLabel.Text = string.format(
                        Slider.Style == "Square" and "%s%s%s / %s%s%s" or "%s%s%s/%s%s%s",
                        Slider.Prefix,
                        Slider.Value,
                        Slider.Suffix,
                        Slider.Prefix,
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        local Dropdown = {
            ...
            ValueFonts = typeof(Info.ValueFonts) == "table" and Info.ValueFonts or nil,
            FontPreview = Info.FontPreview == true,
        ...
        local GetValueFont = function(Value, RawValue)
            local Ref = nil
            if Dropdown.ValueFonts then
                Ref = Dropdown.ValueFonts[Value]
                if Ref == nil and RawValue ~= nil then
                    Ref = Dropdown.ValueFonts[RawValue]
                end
            end
            if Ref == nil and Dropdown.FontPreview then
                Ref = if RawValue ~= nil then RawValue else Value
            end

            return Library:ResolveFont(Ref)
        end

        local function ApplyTextFont(Object, Face)
            local Registry = Library.Registry[Object]
            if Face then
                if Registry then
                    Registry.FontFace = nil
                end
                Object.FontFace = Face
            else
                if Registry then
                    Registry.FontFace = "Font"
                else
                    Library.Registry[Object] = { FontFace = "Font" }
                end
        ...
        function Dropdown:Display()
            ...
            if Info.Multi then
                ...
                DisplayFace = GetValueFont(Dropdown.Value, DisplayValue)
            ...
            ApplyTextFont(DisplayButton, DisplayFace)
        ...
        local function RecomputeFilteredEntries()
            ...
            for Key, RawValue in Values do
                ...
                local Entry = {
                    ...
                    ValueImage = GetValueImage(Value, RawValue),
                    ValueFont = GetValueFont(Value, RawValue),
                    SortKey = Key,
        ...
        function Dropdown:RefreshPool()
            ...
            for SlotIndex, Row in Pool do
                ...
                ApplyTextFont(Row.Button, Entry.ValueFont)
        ...
        function Dropdown:SetValueFonts(ValueFonts)
            Dropdown.ValueFonts = typeof(ValueFonts) == "table" and ValueFonts or nil
            Dropdown:BuildDropdownList()
            Dropdown:Display()
        end

        function Dropdown:SetFontPreview(State: boolean)
            Dropdown.FontPreview = State == true
            Dropdown:BuildDropdownList()
            Dropdown:Display()
        end
    ...
    function Funcs:AddCard(Idx, Info)
        if self.Destroyed then return nil end
        ...
        local StatusColors = {
            ...
            DoNotDisturb = Color3.fromRGB(242, 63, 67),
        ...
        local Card = {
            ...
            Banner = Info.Banner,
            BannerHeight = math.max(24, tonumber(Info.BannerHeight) or 64),
            Avatar = Info.Avatar,
            AvatarPending = false,
            Status = Info.Status,
            Buttons = {},
        ...
        local Tokens = { Banner = 0, Avatar = 0, Invite = 0 }
        local BannerCorners = {}
        ...
        local BannerFill = New("Frame", {
            BackgroundColor3 = "AccentColor",
            Size = UDim2.new(1, 0, 0, Card.BannerHeight),
            ZIndex = 1,
            Parent = Box,
        })
        New("UIGradient", {
            Color = ColorSequence.new(Color3.new(1, 1, 1), Color3.fromRGB(96, 96, 96)),
            Rotation = 25,
            Parent = BannerFill,
        })
        ...
        for _, Target in { BannerFill, Banner } do
            table.insert(BannerCorners, New("UICorner", {
                TopLeftRadius = UDim.new(0, Library.CornerRadius / 2),
                TopRightRadius = UDim.new(0, Library.CornerRadius / 2),
                BottomLeftRadius = UDim.new(0, 0),
                BottomRightRadius = UDim.new(0, 0),
        ...
        local AvatarRing = New("Frame", {
            BackgroundColor3 = "MainColor",
            Position = UDim2.fromOffset(7, Card.BannerHeight - 34),
        ...
        local StatusDot = New("Frame", {
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundColor3 = StatusColors.Online,
            Position = UDim2.fromScale(0.5, 0.5),
            Size = UDim2.new(1, -4, 1, -4),
            ZIndex = 6,
            Parent = StatusRing,
        ...
        local Body = New("Frame", {
            BackgroundTransparency = 1,
            Position = UDim2.fromOffset(0, Card.BannerHeight),
            Size = UDim2.new(1, 0, 1, -Card.BannerHeight),
            Parent = Box,
        })
        New("UIPadding", {
        ...
        function Card:Refresh()
            ...
            local BannerHeight = Card.BannerHeight
            local HasAvatar = AvatarRing.Visible
            local HasDescription = typeof(Card.Description) == "string" and Card.Description ~= ""
            local HasButtons = #Card.Buttons > 0
            local SpacerHeight = if HasAvatar then 32 else 4

            BannerFill.Size = UDim2.new(1, 0, 0, BannerHeight)
            Banner.Size = UDim2.new(1, 0, 0, BannerHeight)
            AvatarRing.Position = UDim2.fromOffset(7, BannerHeight - 34)
            Body.Position = UDim2.fromOffset(0, BannerHeight)
            Body.Size = UDim2.new(1, 0, 1, -BannerHeight)
            Spacer.Size = UDim2.new(1, 0, 0, SpacerHeight)

            TitleLabel.Text = tostring(Card.Title or "")
            DescriptionLabel.Text = if HasDescription then Card.Description else ""
            DescriptionLabel.Visible = HasDescription
            ButtonRow.Visible = HasButtons

            local Content = SpacerHeight + 2 + 18
            ...
            local Radius = UDim.new(0, Library.CornerRadius / 2)
            for _, Corner in BannerCorners do
                Corner.TopLeftRadius = Radius
                Corner.TopRightRadius = Radius
            end

            Holder.Size = UDim2.new(1, 0, 0, BannerHeight + Content + 10)
            Groupbox:Resize()
        end

        function Card:SetTitle(Title: string?)
            Card.Title = Title
            Card.Text = Title
            Card:Refresh()
        end

        function Card:SetDescription(Description: string?)
            Card.Description = Description
            Card:Refresh()
        end

        function Card:SetBannerHeight(Height: number)
            Card.BannerHeight = math.max(24, tonumber(Height) or Card.BannerHeight)
            Card:Refresh()
        end

        function Card:SetBanner(Ref)
            Card.Banner = Ref
            Tokens.Banner += 1
            local Token = Tokens.Banner

            Banner.Image = ""
            Library:ResolveImage(Ref, function(Asset)
                if Token ~= Tokens.Banner or Card.Destroyed then
                    return
                end
                Banner.Image = Asset or ""
            end)
        end

        function Card:SetAvatar(Ref)
            Card.Avatar = Ref
            Tokens.Avatar += 1
            local Token = Tokens.Avatar

            local Shown = (Ref ~= nil and Ref ~= false and Ref ~= "") or Card.AvatarPending
            local Changed = AvatarRing.Visible ~= Shown
            AvatarRing.Visible = Shown
            AvatarImage.Image = ""

            Library:ResolveImage(Ref, function(Asset)
                if Token ~= Tokens.Avatar or Card.Destroyed then
                    return
                end
                AvatarImage.Image = Asset or ""
            end)
        ...
        function Card:SetStatus(Status)
            Card.Status = Status

            local Color = if typeof(Status) == "Color3" then Status else StatusColors[Status]
        ...
        function Card:SetButtons(List)
            for _, Button in Card.Buttons do
                Button.Instance:Destroy()
            end
            table.clear(Card.Buttons)

            if typeof(List) == "table" then
                for Order, Spec in List do
                    ...
                    local Button = {
                        ...
                        Icon = Spec.Icon,
                        Copy = Spec.Copy,
                        CopiedText = Spec.CopiedText or "Copied",
                        Callback = Spec.Callback,
                    }

                    local Base = New("TextButton", {
                        AutoButtonColor = false,
                        BackgroundColor3 = if typeof(Spec.Color) == "Color3" then Spec.Color else "AccentColor",
                        LayoutOrder = Order,
                    ...
                    Button.Instance = Base
                    Button.Label = Label

                    local function ShowIcon(Ref)
                        local Icon = Ref and Library:GetCustomIcon(Ref)
                        IconImage.Visible = Icon ~= nil
                        if Icon then
                            Library:ApplyLucideIcon(IconImage, Icon)
                        end
                    end
                    ShowIcon(Button.Icon)
                    ...
                    function Button:Press()
                        if Button.Copy ~= nil and setclipboard then
                            pcall(setclipboard, tostring(Button.Copy))

                            CopyToken += 1
                            local Token = CopyToken
                            Label.Text = Button.CopiedText
                            ShowIcon("check")

                            task.delay(1.5, function()
                                if Token ~= CopyToken or Card.Destroyed or not Base.Parent then
                                    return
                                end
                                Label.Text = Button.Text
                                ShowIcon(Button.Icon)
                        ...
                        if typeof(Button.Callback) == "function" then
                            Library:SafeCallback(Button.Callback, Button)
                        end
                    end

                    local Hovering = false
                    table.insert(Card.Connections, Base.MouseEnter:Connect(function()
                        Hovering = true
                        TweenService:Create(Overlay, Library.TweenInfo, { BackgroundTransparency = 0.9 }):Play()
                    end))
                    table.insert(Card.Connections, Base.MouseLeave:Connect(function()
                        Hovering = false
                        TweenService:Create(Overlay, Library.TweenInfo, { BackgroundTransparency = 1 }):Play()
                    end))
                    table.insert(Card.Connections, Base.MouseButton1Down:Connect(function()
                        TweenService:Create(Overlay, Library.TweenInfo, { BackgroundTransparency = 0.82 }):Play()
                    end))
                    table.insert(Card.Connections, Base.MouseButton1Up:Connect(function()
                        TweenService:Create(Overlay, Library.TweenInfo, { BackgroundTransparency = if Hovering then 0.9 else 1 }):Play()
                    end))
                    table.insert(Card.Connections, Base.MouseButton1Click:Connect(function()
                        Button:Press()
                    end))
        ...
        function Card:SetInvite(Code)
            Tokens.Invite += 1
            local Token = Tokens.Invite

            local Normalized = Library:NormalizeInvite(Code)
            Card.Invite = Normalized
            Card.InviteData = nil
            if not Normalized then
                return
            end

            local Link = "https://discord.gg/" .. Normalized
            if Info.Title == nil then
                Card.Title = "discord.gg/" .. Normalized
            ...
            if Info.Avatar == nil then
                Card.AvatarPending = true
                Card:SetAvatar(nil)
            end
            if Info.Buttons == nil then
                Card:SetButtons({
                    { Text = "Copy Discord Invite", Icon = "copy", Copy = Link, Color = Color3.fromRGB(88, 101, 242) },
                })
            end
            Card:Refresh()

            task.spawn(function()
                local Data = Library:FetchDiscordInvite(Normalized)
                if Token ~= Tokens.Invite or Card.Destroyed then
                    return
                end

                Card.InviteData = Data
                if Info.Avatar == nil then
                    Card.AvatarPending = false
                end
                ...
                if Info.Title == nil and Data.Name then
                ...
                if Info.Description == nil then
                    local Parts = {}
                    if Data.Online then
                        table.insert(Parts, FormatCount(Data.Online) .. " online")
                    end
                    if Data.Members then
                        table.insert(Parts, FormatCount(Data.Members) .. " members")
                    end
                    Card.Description = table.concat(Parts, " · ")
                end
                if Info.Status == nil then
                    Card:SetStatus("Online")
                end
                if Info.Avatar == nil then
                    Card:SetAvatar(Data.Icon)
                end
                if Info.Banner == nil and Data.Banner then
                    Card:SetBanner(Data.Banner)
                end
                Card:Refresh()
            end)
        end

        function Card:RefreshInvite()
            if Card.Invite then
                Card:SetInvite(Card.Invite)
            end
        end
        ...
        Card:SetAvatar(Info.Avatar)
        Card:SetStatus(Info.Status)
        Card:SetButtons(Info.Buttons)
        if Info.Invite then
            Card:SetInvite(Info.Invite)
        end
        Card:Refresh()
        ...
        function Card:Destroy()
            ...
            Groupbox:Resize()
            if Idx ~= nil and Options[Idx] == Card then
                Options[Idx] = nil
            end
        end
...
function Library:ResolveFont(Ref): Font?
    local RefType = typeof(Ref)

    if RefType == "Font" then
        return Ref
    elseif RefType == "EnumItem" then
        local Ok, Result = pcall(Font.fromEnum, Ref)
        return Ok and Result or nil
    elseif RefType == "number" then
        local Ok, Result = pcall(Font.fromId, Ref)
        return Ok and Result or nil
    elseif RefType ~= "string" or Ref == "" then
        return nil
    end

    local Cached = Library.FontCache[Ref]
    if Cached ~= nil then
    ...
    local Ok, Item = pcall(function()
        return Enum.Font[Ref]
    end)
    if Ok and Item then
        Result = Font.fromEnum(Item)
    elseif Ref:find("^rbxasset") then
        local Created, Face = pcall(Font.new, Ref)
        Result = Created and Face or nil
    end

    Library.FontCache[Ref] = Result or false
    return Result
end

function Library:DownloadImage(Url: string): string?
    local Cached = Library.ImageCache[Url]
    if Cached ~= nil then
        return Cached or nil
    end
    ...
    local Hash = 5381
    for Index = 1, #Url do
        Hash = (Hash * 33 + string.byte(Url, Index)) % 4294967296
    end
    local Extension = Url:match("%.(%a+)%?") or Url:match("%.(%a+)$") or "png"
    Extension = Extension:lower()
    if Extension ~= "png" and Extension ~= "jpg" and Extension ~= "jpeg" then
        Extension = "png"
    end

    local Folder = "Obsidian/remote_images"
    local Path = string.format("%s/%08x.%s", Folder, Hash, Extension)
    ...
    local Ok, Asset = pcall(function()
        ...
        if not isfile(Path) then
            ...
            assert(typeof(Body) == "string" and #Body > 64, "empty body")
            writefile(Path, Body)
        end
        return getcustomasset(Path)
    end)

    Library.ImageCache[Url] = Ok and Asset or false
    return Ok and Asset or nil
end

function Library:ResolveImage(Ref, Callback: (string?) -> ())
    if Ref == nil or Ref == false or Ref == "" then
        Callback(nil)
        return
    end

    if tonumber(Ref) then
        Callback(string.format("rbxassetid://%s", tostring(Ref)))
        return
    end
    ...
    if Ref:find("^https?://") then
        local Cached = Library.ImageCache[Ref]
        if Cached ~= nil then
            Callback(Cached or nil)
            return
        end
        task.spawn(function()
            Callback(Library:DownloadImage(Ref))
        end)
        return
    end

    if Ref:find("^rbx") or Ref:find("^content://") then
        Callback(Ref)
        return
    end

    local Icon = Library:GetCustomIcon(Ref)
    Callback(Icon and Icon.Url or nil)
end

function Library:NormalizeInvite(Code): string?
    ...
    local Text = tostring(Code)
        ...
        :gsub("^discordapp%.com/invite/", "")
        :gsub("[/?#].*$", "")

    return if Text ~= "" then Text else nil
end

function Library:FetchDiscordInvite(Code: string)
    Code = Library:NormalizeInvite(Code)
    if not Code then
        return nil
    end

    local Ok, Data = pcall(function()
        local Body = game:HttpGet("https://discord.com/api/v10/invites/" .. Code .. "?with_counts=true")
        return cloneref(game:GetService("HttpService")):JSONDecode(Body)
    end)
    if not Ok or typeof(Data) ~= "table" or typeof(Data.guild) ~= "table" then
        return nil
    end

    local Guild = Data.guild
    local function CdnUrl(Kind, Hash, Size)
        if typeof(Hash) ~= "string" or Hash == "" then
            return nil
        end
        return string.format("https://cdn.discordapp.com/%s/%s/%s.png?size=%d", Kind, tostring(Guild.id), Hash, Size)
    end
    ...
    return {
        ...
        Icon = CdnUrl("icons", Guild.icon, 128),
        Banner = CdnUrl("banners", Guild.banner, 480) or CdnUrl("splashes", Guild.splash, 480),
        Members = tonumber(Data.approximate_member_count),
        Online = tonumber(Data.approximate_presence_count),
    }
end
...
function Library:AttachShadow(Target: GuiObject, Info)
    ...
    function Shadow:Sync()
        local Parent = Image.Parent
        if not (Parent and Target.Parent) then
            return
        end

        local Blur = math.max(1, Shadow.Blur)
        local Extent = Shadow.Spread + Blur
        local Root = if Parent:IsA("GuiBase2d") then Parent.AbsolutePosition else Vector2.zero
        ...
        Image.Visible = Shadow.Enabled and Target.Visible

        local Fade = math.clamp(Target.BackgroundTransparency, 0, 1)
        Image.ImageTransparency = Shadow.Transparency + (1 - Shadow.Transparency) * Fade
    end

    local function ApplyColor()
        local Color = Shadow.Color
        local Registry = Library.Registry[Image]

        if typeof(Color) == "string" then
            if not Registry then
                Registry = {}
                Library.Registry[Image] = Registry
            end
            Registry.ImageColor3 = Color
            Image.ImageColor3 = Library.Scheme[Color] or Color3.new(0, 0, 0)
        else
            if Registry then
                Registry.ImageColor3 = nil
            end
            Image.ImageColor3 = if typeof(Color) == "Color3" then Color else Color3.new(0, 0, 0)
        end
    end

    function Shadow:GetStyle()
        return {
    ...
    function Shadow:SetStyle(Style)
        if typeof(Style) == "string" then
            Style = Library.ShadowPresets[Style]
        end
        ...
        if Style.Enabled ~= nil then
            Shadow.Enabled = Style.Enabled == true
        end
        if tonumber(Style.Transparency) then
            Shadow.Transparency = math.clamp(tonumber(Style.Transparency), 0, 1)
        end
        if tonumber(Style.Blur) then
            Shadow.Blur = math.clamp(tonumber(Style.Blur), 1, 200)
        end
        if tonumber(Style.Spread) then
            Shadow.Spread = math.clamp(tonumber(Style.Spread), -50, 100)
        end
        if typeof(Style.Offset) == "Vector2" then
            Shadow.Offset = Style.Offset
        end
        if tonumber(Style.OffsetX) or tonumber(Style.OffsetY) then
            Shadow.Offset = Vector2.new(tonumber(Style.OffsetX) or Shadow.Offset.X, tonumber(Style.OffsetY) or Shadow.Offset.Y)
        end
        if typeof(Style.Color) == "Color3" or typeof(Style.Color) == "string" then
    ...
    function Shadow:SetEnabled(State: boolean)
        Shadow:SetStyle({ Enabled = State == true })
    end

    function Shadow:SetTransparency(Value: number)
        Shadow:SetStyle({ Transparency = Value })
    end

    function Shadow:SetBlur(Value: number)
        Shadow:SetStyle({ Blur = Value })
    end

    function Shadow:SetSpread(Value: number)
        Shadow:SetStyle({ Spread = Value })
    end

    function Shadow:SetOffset(X, Y)
        if typeof(X) == "Vector2" then
            Shadow:SetStyle({ Offset = X })
        else
            Shadow:SetStyle({ OffsetX = X, OffsetY = Y })
        end
    end

    function Shadow:SetColor(Color)
        Shadow:SetStyle({ Color = Color })
    end
    ...
    for _, Property in { "AbsolutePosition", "AbsoluteSize", "Visible", "BackgroundTransparency", "ZIndex" } do
        table.insert(Shadow.Connections, Library:GiveSignal(Target:GetPropertyChangedSignal(Property):Connect(function()
            Shadow:Sync()
        end)))
    end
    table.insert(Shadow.Connections, Library:GiveSignal(Target.Destroying:Connect(function()
        Shadow:Destroy()
    end)))

    ApplyColor()
    Shadow:SetStyle(Info)
    return Shadow
end
...
function Library:SetToggleStyle(Style: string)
    Library.ToggleStyle = Style == "Square" and "Square" or "Round"

    for _, Toggle in Toggles do
        if Toggle.Variant == "Switch" and not Toggle.StyleExplicit and typeof(Toggle.SetStyle) == "function" then
            Toggle:SetStyle(Library.ToggleStyle)
        end
    end
end

function Library:SetSliderStyle(Style: string)
    Library.SliderStyle = Style == "Square" and "Square" or "Default"

    for _, Option in Options do
        if Option.Type == "Slider" and not Option.StyleExplicit and typeof(Option.SetStyle) == "function" then
            Option:SetStyle(Library.SliderStyle)
        end
    end
end

function Library:BuildTabDecor(TabButton: GuiObject, Style, HasIcon: boolean)
    local Indicator, Chip, ChipStroke
    ...
    if Style.Marker then
        Indicator = New("Frame", {
            ...
            BackgroundColor3 = "AccentColor",
            BackgroundTransparency = 1,
            Position = UDim2.new(0, Style.MarkerOffset, 0.5, 0),
            Size = UDim2.fromOffset(Style.MarkerWidth, Style.MarkerIdleHeight),
            ZIndex = 2,
            Parent = TabButton,
        })
        New("UICorner", {
    ...
    elseif Style.Indicator then
        Indicator = New("Frame", {
            ...
            Size = UDim2.fromOffset(Style.IndicatorWidth, Style.IndicatorHeight),
            Parent = TabButton,
        })
        New("UICorner", {
            CornerRadius = UDim.new(1, 0),
    ...
    if Style.Chip and HasIcon then
        Chip = New("Frame", {
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundColor3 = "AccentColor",
            BackgroundTransparency = 1,
            Position = UDim2.fromScale(0.5, 0.5),
            Size = UDim2.fromOffset(Style.ChipIdleSize, Style.ChipIdleSize),
            Visible = false,
            ZIndex = 0,
            Parent = TabButton,
        })
        New("UICorner", {
            CornerRadius = UDim.new(0, Style.ChipCornerRadius),
            Parent = Chip,
        })
        New("UIGradient", {
            Color = ColorSequence.new(Color3.new(1, 1, 1), Color3.fromRGB(194, 194, 194)),
            Rotation = 90,
            Parent = Chip,
        })
        ChipStroke = New("UIStroke", {
            ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
            Color = Color3.new(1, 1, 1),
            Thickness = 1,
            Transparency = 1,
    ...
    return Indicator, Chip, ChipStroke
end

function Library:PaintTabButton(Entry, Animate: boolean?)
    local Style = Entry.Style
    local Active = Entry.Active
    local Collapsed = Entry.Compact and Entry.Icon ~= nil
    local ChipMode = Collapsed and Entry.Chip ~= nil
    local Idle = if Entry.Hovering then 0.25 else 0.5
    ...
    Apply(Entry.Button, { BackgroundTransparency = if Active and not ChipMode then 0 else 1 })
    Apply(Entry.Label, { TextTransparency = if Active then 0 else Idle })

    if Entry.Indicator then
        local Properties = { BackgroundTransparency = if Active and not (Collapsed and (Style.Marker or ChipMode)) then 0 else 1 }
        if Style.Marker then
            Properties.Size = UDim2.fromOffset(Style.MarkerWidth, if Active then Style.MarkerHeight else Style.MarkerIdleHeight)
        end
        Apply(Entry.Indicator, Properties)
    end

    if Entry.Chip then
        local Size = if Active then Style.ChipSize else Style.ChipIdleSize
        Entry.Chip.Visible = ChipMode
        Apply(Entry.Chip, {
            BackgroundTransparency = if Active then 0 else 1,
            Size = UDim2.fromOffset(Size, Size),
        })
        Apply(Entry.ChipStroke, { Transparency = if Active then 0.55 else 1 })
    end

    if Entry.Icon then
        local Properties = { ImageTransparency = if Active then 0 else Idle }
        if not Entry.Custom then
            local ColorKey = if ChipMode and Active then "BackgroundColor" else "AccentColor"
            local Registry = Library.Registry[Entry.Icon]
            if Registry then
                Registry.ImageColor3 = ColorKey
            end
            Properties.ImageColor3 = Library.Scheme[ColorKey]
        end
        Apply(Entry.Icon, Properties)
    end
end

function Library:LayoutTabButton(Entry, Compact: boolean)
    if not Entry.Icon then
        return
    end

    local Pad = if Entry.Chip then Entry.Style.ChipPadding else 6
    Entry.Compact = Compact
    Entry.Label.Visible = not Compact
    Entry.Padding.PaddingBottom = UDim.new(0, if Compact then Pad else 11)
    Entry.Padding.PaddingLeft = UDim.new(0, if Compact then Pad else 12)
    Entry.Padding.PaddingRight = UDim.new(0, if Compact then Pad else 12)
    Entry.Padding.PaddingTop = UDim.new(0, if Compact then Pad else 11)
    Entry.Icon.SizeConstraint = if Compact then Enum.SizeConstraint.RelativeXY else Enum.SizeConstraint.RelativeYY
    Library:PaintTabButton(Entry, false)
end
...
function Library:CreateWindow(WindowInfo)
    ...
    Library.ToggleStyle = WindowInfo.ToggleStyle == "Square" and "Square" or "Round"
    Library.SliderStyle = WindowInfo.SliderStyle == "Square" and "Square" or "Default"
    ...
    local MinimizeState = {
        Reserve = WindowInfo.MinimizeButton and 81 or 49,
        Button = nil,
        Minimized = false,
        Card = nil,
        Placed = false,
    ...
    do
        ...
        Library.WindowShadow = Library:AttachShadow(MainFrame, {
            Enabled = WindowInfo.Shadow ~= false,
            Transparency = WindowInfo.ShadowTransparency,
            Blur = WindowInfo.ShadowBlur,
        ...
        RightWrapper = New("Frame", {
            AnchorPoint = Vector2.new(1, 0.5),
            BackgroundTransparency = 1,
            Position = UDim2.new(1, -MinimizeState.Reserve, 0.5, 0),
            Size = UDim2.new(1, -InitialLeftWidth - MinimizeState.Reserve - 8 - 1, 1, -16),
            Parent = TopBar,
        })
        ...
        if WindowInfo.MinimizeButton then
            MinimizeState.Button = New("ImageButton", {
                AnchorPoint = Vector2.new(1, 0.5),
                AutoButtonColor = false,
                BackgroundTransparency = 1,
                ImageColor3 = "OutlineColor",
            ...
            if MinimizeIcon then
                Library:ApplyLucideIcon(MinimizeState.Button, MinimizeIcon)
            end

            Library:GiveSignal(MinimizeState.Button.MouseEnter:Connect(function()
                Library.Registry[MinimizeState.Button].ImageColor3 = "FontColor"
                TweenService:Create(MinimizeState.Button, Library.TweenInfo, {
                    ImageColor3 = Library.Scheme.FontColor,
                }):Play()
            end))

            Library:GiveSignal(MinimizeState.Button.MouseLeave:Connect(function()
                Library.Registry[MinimizeState.Button].ImageColor3 = "OutlineColor"
                TweenService:Create(MinimizeState.Button, Library.TweenInfo, {
                    ImageColor3 = Library.Scheme.OutlineColor,
                }):Play()
            end))

            Library:GiveSignal(MinimizeState.Button.MouseButton1Click:Connect(function()
                if MinimizeState.Toggle then
                    MinimizeState.Toggle()
                end
            end))
        end
        ...
        MinimizeState.FooterCorner = New("UICorner", {
            TopLeftRadius = UDim.new(0, 0),
            TopRightRadius = UDim.new(0, 0),
    ...
    function Window:IsMinimized(): boolean
        return MinimizeState.Minimized
    end

    function Window:SetShadowStyle(Style)
        for _, Shadow in { Library.WindowShadow, MinimizeState.CardShadow } do
            Shadow:SetStyle(Style)
        end
    end

    function Window:SetShadow(State: boolean, Transparency: number?)
        Window:SetShadowStyle({ Enabled = State == true, Transparency = Transparency })
    end

    function Window:GetShadowStyle()
    ...
    function MinimizeState.Build()
        ...
        local CardIcon = WindowInfo.Icon and Library:GetCustomIcon(WindowInfo.Icon)
        if CardIcon then
            local CardIconImage = New("ImageLabel", {
                AnchorPoint = Vector2.new(0.5, 0.5),
                BackgroundTransparency = 1,
                Position = UDim2.fromScale(0.5, 0.5),
                Size = UDim2.fromOffset(16, 16),
            ...
            Library:ApplyLucideIcon(CardIconImage, CardIcon)
        else
            New("TextLabel", {
                BackgroundTransparency = 1,
                Size = UDim2.fromScale(1, 1),
        ...
        MinimizeState.Title = New("TextLabel", {
            BackgroundTransparency = 1,
        ...
        MinimizeState.Subtitle = New("TextLabel", {
            BackgroundTransparency = 1,
            Size = UDim2.new(1, 0, 0, 14),
            Text = "",
            TextSize = 12,
        ...
        local ExpandImage = New("ImageLabel", {
            AnchorPoint = Vector2.new(0.5, 0.5),
            BackgroundTransparency = 1,
            ImageColor3 = "FontColor",
        ...
        local ExpandIcon = Library:GetIcon("chevron-up")
        if ExpandIcon then
            Library:ApplyLucideIcon(ExpandImage, ExpandIcon)
        end
        Library:GiveSignal(Expand.MouseEnter:Connect(function()
            TweenService:Create(ExpandImage, Library.TweenInfo, { ImageTransparency = 0 }):Play()
        end))
        Library:GiveSignal(Expand.MouseLeave:Connect(function()
            TweenService:Create(ExpandImage, Library.TweenInfo, { ImageTransparency = 0.35 }):Play()
        end))
        Library:GiveSignal(Expand.MouseButton1Click:Connect(function()
            Window:SetMinimized(false)
        end))
        MinimizeState.Expand = Expand
        ...
        MinimizeState.Footer = New("TextLabel", {
            BackgroundTransparency = 1,
            Size = UDim2.fromScale(1, 1),
            Text = "",
            TextSize = 12,
        ...
        Library:MakeDraggable(Card, Header, false, false)

        MinimizeState.Card = Card
        return Card
    end

    function MinimizeState.Refresh()
        if not MinimizeState.Card then
            return
        end
        ...
        for Name, Tab in Library.Tabs do
            if Tab == Library.ActiveTab then
                TabName = tostring(Name)
                break
            end
        end

        MinimizeState.Title.Text = "<b>" .. WindowTitle.Text .. "</b>"
        MinimizeState.Subtitle.Text = TabName
        MinimizeState.Footer.Text = FooterLabel.Text
    end

    function Window:SetMinimized(State: boolean)
        State = State == true
        if State == MinimizeState.Minimized then
            return
        end
        MinimizeState.Minimized = State

        local Card = MinimizeState.Build()

        if State then
            MinimizeState.Refresh()

            if not MinimizeState.Placed then
                MinimizeState.Placed = true

                local Origin = ScreenGui.AbsolutePosition
                local Width = 300 * Library.DPIScale
                local Right = MainFrame.AbsolutePosition.X + MainFrame.AbsoluteSize.X - Origin.X
                Card.Position = UDim2.fromOffset(
                    math.max(6, Right - Width),
                    math.max(6, MainFrame.AbsolutePosition.Y - Origin.Y)
                )
            end

            MainFrame.Visible = false
            Card.Visible = Library.Toggled
    ...
    function Window:ToggleMinimized()
        Window:SetMinimized(not MinimizeState.Minimized)
    end

    MinimizeState.Toggle = function()
    ...
    function Window:SetCornerRadius(Radius: number)
        ...
        MinimizeState.FooterCorner.BottomLeftRadius = RadiusUDim
        MinimizeState.FooterCorner.BottomRightRadius = RadiusUDim
    ...
    local function ApplyCompact()
        ...
        for _, Button in Library.TabButtons do
            if Button.Compact ~= IsCompact then
                Library:LayoutTabButton(Button, IsCompact)
            end
        end
    end
    ...
    function Window:SetSidebarWidth(Width)
        ...
        RightWrapper.Size = UDim2.new(1, -Width - MinimizeState.Reserve - 8 - 1, 1, -16)
    ...
    function Window:AddTab(...)
        ...
        do
            ...
            local TabIndicator, TabChip, TabChipStroke = Library:BuildTabDecor(TabButton, TabButtonsStyle, Icon ~= nil)
            ...
            TabEntry = {
                ...
                Icon = TabIcon,
                Custom = Icon ~= nil and Icon.Custom == true,
                Indicator = TabIndicator,
                Chip = TabChip,
                ChipStroke = TabChipStroke,
                Style = TabButtonsStyle,
                Active = false,
                Hovering = false,
                Compact = false,
            }
            table.insert(Library.TabButtons, TabEntry)
            Library:LayoutTabButton(TabEntry, IsCompact)
        ...
        function Tab:Hover(Hovering)
            TabEntry.Hovering = Hovering
            if Library.ActiveTab == Tab then
                return
            end

            Library:PaintTabButton(TabEntry, true)
        end
    ...
    function Window:Toggle(Value: boolean?)
        ...
        if MinimizeState.Minimized then
            MainFrame.Visible = false
            if MinimizeState.Card then
                MinimizeState.Card.Visible = Library.Toggled
            end
        elseif Library.Animations and Library.Animations.ToggleWindow == true then
            ...
            task.delay(FadeTime, function()
                MainFrame.Visible = Library.Toggled and not MinimizeState.Minimized
                Fading = false
            end)
        else
            MainFrame.Visible = Library.Toggled
    ...
    if WindowInfo.Minimized then
        Window:SetMinimized(true, true)
    end

    Library.MobileLayout = {
        Mode = WindowInfo.MobileLayout,
        Active = false,
        Area = nil,
        ManualScale = false,
        DesktopSize = MainFrame.Size,
        DesktopScale = Library.DPIScale,
        DesktopCompact = nil,
        Pending = false,
    }

    function Window:GetScreenArea(): Vector2
        local Layout = Library.MobileLayout
        if Layout.Area then
            return Layout.Area
        end
    ...
    function Window:IsMobileLayout(): boolean
        return Library.MobileLayout.Active
    end

    function Window:FitToScreen()
        local Layout = Library.MobileLayout
        local Area = Window:GetScreenArea()
        local Portrait = Area.Y > Area.X
        local Small = math.min(Area.X, Area.Y) < 520

        local BaseSize = if Small then WindowInfo.MobileSize else Layout.DesktopSize
        local Base = Vector2.new(BaseSize.X.Offset, BaseSize.Y.Offset)
        local Target = Vector2.new(
            Area.X * (if Portrait then 0.94 else WindowInfo.MobileWidth),
            Area.Y * (if Portrait then 0.6 else WindowInfo.MobileHeight)
        )

        local Scale = Library.DPIScale
        if not Layout.ManualScale then
            Scale = math.min(Target.X / Base.X, Target.Y / Base.Y)
            Scale = math.clamp(Scale, WindowInfo.MobileMinScale, WindowInfo.MobileMaxScale)
            Scale = math.floor(Scale * 100) / 100
            Library:SetDPIScale(Scale * 100, true)
        end

        local Width = math.floor(math.min(Base.X, Target.X / Scale))
        local Height = math.floor(math.min(Base.Y, Target.Y / Scale))
        MainFrame.Size = UDim2.fromOffset(Width, Height)
        MainFrame.Position = UDim2.new(0.5, -math.floor(Width * Scale / 2), 0.5, -math.floor(Height * Scale / 2))

        return Scale, Width, Height
    end

    function Window:SetMobileLayout(Mode)
        local Layout = Library.MobileLayout
        if Mode ~= nil then
            Layout.Mode = Mode
        end

        local Active = Layout.Mode == true or (Layout.Mode == "Auto" and Library.IsMobile) or Layout.Area ~= nil
        if Layout.Mode == false then
            Active = false
        end

        if Active then
            if not Layout.Active then
                Layout.DesktopSize = MainFrame.Size
                Layout.DesktopScale = Library.DPIScale
                Layout.DesktopCompact = Window:IsSidebarCompacted()
                Layout.ManualScale = false
                Layout.Active = true
                CurrentTabDescription.Visible = false
                Window:FitToScreen()
                if WindowInfo.EnableCompacting and WindowInfo.MobileCompact then
                    Window:SetCompact(true)
                end
            else
                Window:FitToScreen()
            end
        elseif Layout.Active then
            Layout.Active = false
            Layout.ManualScale = false
            CurrentTabDescription.Visible = true
            Library:SetDPIScale(Layout.DesktopScale * 100, true)

            local Size = Layout.DesktopSize
            MainFrame.Size = Size
            MainFrame.Position = UDim2.new(
                0.5,
                -math.floor(Size.X.Offset * Library.DPIScale / 2),
                0.5,
                -math.floor(Size.Y.Offset * Library.DPIScale / 2)
            )

            if WindowInfo.EnableCompacting and Layout.DesktopCompact ~= nil then
                Window:SetCompact(Layout.DesktopCompact)
            end
        end
    end

    function Window:SetMobilePreview(Area: Vector2?)
        Library.MobileLayout.Area = if typeof(Area) == "Vector2" then Area else nil
        if Library.MobileLayout.Area then
            Window:SetMobileLayout(true)
        else
            Window:SetMobileLayout(WindowInfo.MobileLayout)
        end
    end

    Library:GiveSignal(ScreenGui:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
        local Layout = Library.MobileLayout
        if not Layout.Active or Layout.Area or Layout.Pending then
            return
        end
        ...
        task.delay(0.2, function()
            ...
            if Layout.Active and not Layout.Area and not Library.Unloaded then
                Window:FitToScreen()
            end
        end)
    end))

    Window:SetMobileLayout()
